...
async function run(command: Command): Promise<number> {
  ...
  switch (command.name) {
    ...
    case 'explain': {
      if (command.all) {
        process.stdout.write(
          command.json
            ...
            : Object.values(REMEDIES)
                .map(
                  (r) => `${r.code} [${r.category}, ${r.severity}]\n  ${r.meaning}\n  fix: ${r.fix}\n  api: ${r.api}`,
                )
                .join('\n\n') + '\n',
        );
        return 0;
      ...
      if (!remedy)
        throw new UsageError(`unknown hint code "${command.code}"; known: ${Object.keys(REMEDIES).join(', ')}`);
      process.stdout.write(
        command.json
          ? JSON.stringify(remedy, null, 2) + '\n'
