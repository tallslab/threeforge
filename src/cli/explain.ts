...
export const REMEDIES: Record<string, Remedy> = Object.fromEntries([
  ...
  remedy('transparent-overdraw', 'overdraw', 'warn', 'Transparent fragments per pixel exceed the tier budget: fill rate, not draw calls, is the cost.', 'Use alphaTest cutouts instead of blending for foliage and fences, cap live particles per effect, shrink billboard sizes, and scale the render resolution down on low tiers.', 'material.alphaTest, ledger.measureOverdraw(), renderer.setPixelRatio()'),
