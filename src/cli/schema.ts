...
const compileReport: Schema = {
  type: 'object',
  description:
    "threeforge CompileReport: before/after counts, groups, skipped meshes with their rule. Left open (additionalProperties: true) rather than enumerated field-by-field, except for the two true counts below; two report fields worth knowing about: the bake summary's `keptCoincidentFaces` (coincident faces the buried pass kept) and `occlusion.skippedSynced` (batches opted out of occlusion culling because they hold synced movers). `skipped` and `groups` list at most 256 entries each (the CLI caps every array a page returns); `skippedCount` and `groupCount` are their true lengths.",
  properties: {
