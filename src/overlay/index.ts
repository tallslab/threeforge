...
export function formatCostRows(frame: FrameSnapshot): string[] {
  ...
  return [
    ...
    `overdraw     ${o.measured ? `${o.opaque.toFixed(2)} opaque · ${o.transparent.toFixed(2)} transparent fragments/px` : 'not measured'} · ${o.transparentSubmissions} transparent`,
    ...
    `js           ${j.renderMs.toFixed(1)} ms render · ${j.frameMs.toFixed(1)} ms frame · ${j.objects} objects · ${j.autoUpdatedMatrices} auto-matrices`,
