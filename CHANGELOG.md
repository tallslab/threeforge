...
- The MCP server (`threeforge mcp`): `inspect_app`'s input schema no longer has a `tier` field (the CLI already refuses `inspect --tier`; the handler now always measures at `tier: 'auto'`). Every numeric bound and cross-field rule (`frames` ≥ 1, `inspect_app`'s tier, `optimize_asset`'s budget/verify pairing, and so on) is now checked once by the same `validateInput` the CLI uses, so a bad value returns the same `{ error, code: 2 }` JSON regardless of which field or tool it came from, instead of the SDK's own plain-text schema error for some fields and threeforge's JSON for others. `optimize_asset.out` must now end in `.glb` or `.gltf` and sit inside the input file's directory or the working directory, and a new `overwrite` input (default `false`) is required to replace a file that already exists — both return `{ error, code: 2 }` (the existing-file case matches `/exists/`). `timeout`'s description now says it bounds each page step (load, every evaluate, the N-frame measurement, `compile()`), not just "the page". Every tool carries `readOnlyHint`/`destructiveHint`/`openWorldHint` annotations (`optimize_asset` is the only one marked destructive; `inspect_app` the only one marked open-world). Closing stdin (the client disconnecting) now closes the MCP connection and lets the server process exit, instead of leaving it running forever: the SDK 1.30 stdio transport only listens for `data`/`error` on stdin, never `end`.
