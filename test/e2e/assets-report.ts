...
export function markdownBlock(input: GateInput): string | null {
  ...
  if (materials !== '')
    return `run under FORGE_ASSETS_MATERIALS=${materials}, which measures a different configuration`;
...
export function renderReport(rows: ReportRow[], backend: string): string {
  ...
  const ok = rows.filter(
    (r) => !r.error && r.unattributed === 0 && (r.diff ?? 1) < 0.5 && r.restored === r.naive,
  ).length;
...
function stampSentence(rows: ReportRow[]): string {
  ...
  if (stamps.size > 1)
    return `from ${stamps.size} different runs (each row's commit and run are in the JSON beside this file)`;
  const [only = ''] = [...stamps];
