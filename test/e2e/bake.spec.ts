...
test('baking the village keeps the pixels and draws one mesh per group', async ({ forge }) => {
  ...
  const r = await forge.page.evaluate(async () => {
    ...
    return {
      ...
      bake: report.bake,
      totals: frame.totals,
      baked: frame.byReason.baked?.submissions ?? 0,
    };
  });
...
async function buildWall(forge: ForgePage, options: { block: boolean; mirrored: boolean }): Promise<void> {
  await forge.page.evaluate(({ block, mirrored }) => {
    ...
    f.scene.traverse((o) => {
      if ((o as { isMesh?: boolean }).isMesh) (o.userData as { forge?: string }).forge = 'static';
    });
...
test('back-to-back sign cards and a floor under a ceiling keep both faces, seen from both sides', async ({ forge }) => {
  ...
  await forge.page.evaluate(() => {
    ...
    f.scene.add(
      card(0xd04040, 'sign-front', false),
      card(0x4060d0, 'sign-back', true),
      slab(0x40b060, 'upper-floor', true),
      slab(0xd0b040, 'lower-ceiling', false),
    );
    f.scene.traverse((o) => {
      if ((o as { isMesh?: boolean }).isMesh) (o.userData as { forge?: string }).forge = 'static';
    });
...
test('a mirrored, normal-mapped mesh baked by bakeGeometries keeps its tangents and its pixels', async ({ forge }) => {
  ...
  const r = await forge.page.evaluate(() => {
    ...
    const result = f.bakeGeometries([
      { geometry: naive.geometry, matrix: naive.matrixWorld, opaque: true, vertexColors: false },
    ]);
    ...
    const wValues = (geometry: InstanceType<typeof T.BufferGeometry>): number[] => {
      ...
      if (!tangent) return [];
      const used = geometry.index
        ? new Set(Array.from(geometry.index.array))
        : new Set(Array.from({ length: tangent.count }, (_, i) => i));
      return [...new Set([...used].map((i) => tangent.getW(i)))].sort();
    };
    return {
      mirrored: naive.matrixWorld.determinant() < 0,
      hasTangent: result.geometry.getAttribute('tangent') !== undefined,
      sourceW: wValues(naive.geometry),
      bakedW: wValues(result.geometry),
...
test('touching BackSide rooms keep their shared wall from inside and from outside', async ({ forge }) => {
  ...
  await forge.page.evaluate(() => {
    ...
    const rooms: Array<[number, number, string]> = [
      [0, 0xc04040, 'room-a'],
      [3, 0x4060c0, 'room-b'],
    ];
    for (const [x, color, name] of rooms) {
      const room = new T.Mesh(
        ...
        new T.MeshStandardMaterial({ color, roughness: 0.9, side: T.BackSide }),
      );
      room.position.x = x;
      room.name = name;
...
test('a tinted duplicate keeps the colour three draws on top; an interchangeable one goes', async ({ forge }) => {
  ...
  await forge.page.evaluate(() => {
    ...
    f.scene.add(
      crate(0xd04040, -1.2, 'crate-red'),
      crate(0x4060d0, -1.2, 'crate-blue'),
      crate(0x40b060, 1.2, 'crate-green-a'),
      crate(0x40b060, 1.2, 'crate-green-b'),
    );
    const sun = new T.DirectionalLight(0xffffff, 2);
    sun.position.set(3, 6, 5);
    f.scene.add(new T.AmbientLight(0xffffff, 0.6), sun);
...
test('a vertexColor() node with vertexColors false stays out of the bake, batched at parity', async ({ forge }) => {
  ...
  await forge.page.evaluate(() => {
    ...
    backdrop.name = 'backdrop';
    f.scene.add(
      box(painted, -1.6, 0.7, 'painted-a'),
      box(painted, 1.6, 0.7, 'painted-b'),
      box(plain, -1.6, 2.3, 'plain-a'),
      box(plain, 1.6, 2.3, 'plain-b'),
      backdrop,
    );
    f.scene.traverse((o) => {
      if ((o as { isMesh?: boolean }).isMesh) (o.userData as { forge?: string }).forge = 'static';
...
test('object-local normals and a displacement map stay out of the bake, batched at parity', async ({ forge }) => {
  ...
  await forge.page.evaluate(() => {
    ...
    const displaced = new T.MeshStandardMaterial({
      color: 0xc8b078,
      roughness: 0.8,
