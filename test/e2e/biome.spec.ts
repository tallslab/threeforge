...
test('the biome (terrain, water, thousands of props, cars, hi-poly rocks) compiles to a few dozen submissions with the same pixels', {
  tag: '@corpus',
}, async ({ forge }) => {
  test.setTimeout(600_000);
  const extra: Record<string, string> = {};
  for (const [k, v] of Object.entries(process.env))
    if (k.startsWith('FORGE_BIOME_Q_') && v) extra[k.slice('FORGE_BIOME_Q_'.length).toLowerCase()] = v;
  await forge.open('biome', {
    freeze: '1',
    density: process.env.FORGE_BIOME_DENSITY ?? '1',
    dynamics: process.env.FORGE_BIOME_DYNAMICS ?? 'batch-sync',
    ...extra,
  });
  const naive = await forge.page.evaluate(async () => {
    ...
    return {
      ...
      counts: f.biome!.counts,
      programs: f.registry.stats().programs,
  ...
  console.log(
    JSON.stringify({
      ...
      skipped: compiled.skipped.slice(0, 8),
      diffPct: (diff * 100).toFixed(2),
      compileMs: Math.round(compiled.compileMs),
      triangles: compiled.totals.triangles,
      instances: compiled.totals.instances,
      instancesDrawn: compiled.totals.instancesDrawn,
    }),
  );
