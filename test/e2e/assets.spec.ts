...
for (const asset of assets) {
  test(`asset ${asset.name}`, async ({ forge }) => {
    ...
    const naive = await forge.page.evaluate(() => {
      ...
      f.frame(); // warm-up frame: reflectors and other nested passes settle one frame late
      const frame = f.frame();
...
test.afterAll(() => {
  ...
  const rows = JSON.parse(readFileSync(reportPath, 'utf8')) as Row[];
  ...
  writeFileSync('docs/assets-report.md', `# Public asset report\n\nGenerated by \`pnpm assets:report\` on the WebGL2 backend. ${ok}/${total} assets compile cleanly (0 unattributed, < 0.5% pixels changed, decompile restores).\n\n${header}${body}\n`);
});
