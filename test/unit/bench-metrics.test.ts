...
describe('benchMetrics', () => {
  it('maps a snapshot to the pnpm bench metric keys', () => {
    ...
    expect(metricsOf(frame, 2.5, 16.7, 0.5, [4096, 0, 4096], 3)).toEqual({
      sceneSubmissions: 12,
      gpuDraws: 30,
      triangles: 1000,
      programs: 3,
      overdrawOpaque: 1.5,
      overdrawTransparent: 0.25,
      skinnedVertices: 400,
      shadowCasters: 2,
      shadowTexels: 2731,
      textureBytes: 10,
      geometryBytes: 20,
      renderTargetBytes: 30,
      particles: 7,
      fillMegapixels: 0.84,
      objects: 512,
      autoUpdatedMatrices: 12,
      shadowPassesPerFrame: 0.5,
      renderMs: 2.5,
      frameMs: 16.7,
      unattributed: 0,
    });
  ...
  it('keeps the metric key lists in step: the TS source, the node schema, and the gate', () => {
    ...
    expect([...SCHEMA_METRIC_KEYS], "scripts/bench-schema.mjs's copy, in the same wire order").toEqual([
      ...METRIC_KEYS,
    ]);
    expect(
      [...DETERMINISTIC, ...TIMING, 'unattributed'].sort(),
      'every metric is gated, timed, or unattributed',
