...
describe('snapshot v3', () => {
  it('emptyFrame carries every section with zeroed values', () => {
    ...
    expect(f.overdraw).toEqual({
      opaque: 0,
      transparent: 0,
      transparentSubmissions: 0,
      particles: 0,
      pixels: 0,
      measured: false,
    });
    expect(f.skinning).toEqual({
      submissions: 0,
      vertices: 0,
      bones: 0,
      skeletons: 0,
    ...
    expect(f.lighting).toEqual({
      lights: { directional: 0, point: 0, spot: 0, hemisphere: 0, ambient: 0, other: 0 },
      shadowLights: 0,
      shadowPasses: 0,
      shadowCasters: 0,
      shadowTexels: 0,
