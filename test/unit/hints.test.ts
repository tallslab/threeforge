...
describe('tiers', () => {
  ...
  describe('tierInputFromNavigator', () => {
    it('guards a navigator with no userAgentData, deviceMemory or maxTouchPoints', () => {
    ...
    it('returns desktop for a Windows laptop with an RTX 3060 and a touchscreen', () => {
      // The original tier bug.
      const nav = {
        maxTouchPoints: 10,
    ...
    it('returns desktop under WebGPU for a Snapdragon X laptop naming no graphics API', () => {
...
describe('hintsFor', () => {
  ...
  it('counts the draw-call hints by the main-pass objects passed, not by submissions', () => {
    const f = emptyFrame(env);
  ...
  it('counts unsupported-material by the objects passed, by submissions without them', () => {
    const f = emptyFrame(env);
  ...
  it('reports static-unbatched above 20 shared-material statics drawing alone', () => {
    const f = emptyFrame(env);
    f.byReason = {
  ...
  it('collapses several point lights or transmissive meshes into one hint per code', () => {
    // Both scale with the scene.
    const f = emptyFrame(env);
  ...
  describe('transparent-batch-order', () => {
    ...
    it('fires when a transparent batch shares the main pass with a transparent mesh', () => {
      const f = emptyFrame(env);
      const items = [
        { name: 'forge:batch:aa11:0', pass: 'main', reason: 'batched' as const, transparent: true },
    ...
    it('does not fire for the only transparent submission in the main pass', () => {
      const f = emptyFrame(env);
    ...
    it('caps the objects list at 5 names with more transparent batches in the pass', () => {
      const f = emptyFrame(env);
  ...
  // The ledger gathers `localSpaceDraws` on its rescan (the batch-local-space describe in hints-counting.test.ts covers
