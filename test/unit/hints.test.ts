...
describe('tiers', () => {
  ...
  const table: Array<[string, TierInput, Tier]> = [
    ...
    ['Adreno 650 + <=2GB -> phone-low (low-memory downgrade)', { gpu: 'Adreno (TM) 650', deviceMemory: 2 }, 'phone-low'],
    ...
    ['RTX 3060 + touch -> desktop (the original bug: a touch laptop is not a phone)', { gpu: 'ANGLE (NVIDIA, NVIDIA GeForce RTX 3060 Direct3D11 vs_5_0 ps_5_0, D3D11)', touch: true }, 'desktop'],
    ...
    ['bare "Apple" + touch + <=2GB -> phone-low (low-memory downgrade)', { gpu: 'Apple', touch: true, deviceMemory: 2 }, 'phone-low'],
    ...
    ['unknown GPU, mobile:true, <=2GB -> phone-low (low-memory downgrade)', { gpu: 'Unknown Renderer', mobile: true, deviceMemory: 2 }, 'phone-low'],
    ...
    ['Android phone (bare Adreno 740) + touch, mobile undefined -> phone-mid (step 6 still reads the GPU name)', { gpu: 'Adreno (TM) 740', touch: true }, 'phone-mid'],
    ['Adreno 610 + mobile:false -> phone-low (the low-end regex still decides first)', { gpu: 'Adreno (TM) 610', touch: true, mobile: false }, 'phone-low'],
    ...
    ['M2 iPad Pro + touch + <=2GB -> phone-low (low-memory downgrade still applies)', { gpu: 'Apple M2', touch: true, deviceMemory: 2 }, 'phone-low'],
  ...
  describe('tierInputFromNavigator', () => {
    ...
    it('a Windows-on-ARM laptop (Snapdragon X, Adreno through ANGLE) is a desktop, an iPad is not', () => {
      ...
      const snapdragon = tierInputFromNavigator('ANGLE (Qualcomm, Adreno (TM) X1-85 (0x00043050), D3D11)', {
        ...
        userAgentData: { mobile: false }, userAgent: 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
      });
