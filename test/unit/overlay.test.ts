...
describe('formatOverlay v3', () => {
  it('formats the six cost rows and hints from a v3 snapshot', () => {
    ...
    f.lighting = {
      lights: { directional: 1, point: 0, spot: 0, hemisphere: 1, ambient: 0, other: 0 },
      shadowLights: 1,
      shadowPasses: 1,
      shadowCasters: 20,
      shadowTexels: 1_048_576,
      shadowSubmissions: 20,
    };
    f.js = {
      renderMs: 2.4,
      ledgerMs: 0.3,
      frameMs: 16.7,
      objects: 512,
      autoUpdatedMatrices: 12,
      hiddenOriginals: 0,
      skipped: 0,
    };
    f.memory = {
      textures: { count: 8, bytes: 20 * 1024 * 1024 },
      geometries: { count: 30, bytes: 3 * 1024 * 1024 },
      renderTargets: { count: 2, bytes: 5 * 1024 * 1024 },
      unreferenced: { geometries: 0, textures: 0 },
      chunks: { total: 0, resident: 0 },
      measured: null,
      estimated: true,
    };
    f.hints = [
      { category: 'lighting', severity: 'warn', code: 'shadow-texels', message: 'too many shadow texels', objects: [] },
      {
        category: 'js',
        severity: 'info',
        code: 'static-auto-update',
        message: '3 static objects auto-update',
        objects: ['a'],
      },
    ];
    ...
    expect(lines).toContain(
      'js           2.4 ms render · 0.3 ms ledger · 16.7 ms frame · 512 objects · 12 auto-matrices · 0 hidden · 0 skipped',
    );
