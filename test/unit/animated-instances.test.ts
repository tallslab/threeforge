...
import { AnimationMixer, InstancedBufferAttribute, InstancedBufferGeometry, InstancedInterleavedBuffer, Matrix4, Quaternion, Scene, Vector3, Vector4 } from 'three';
...
const placement = (): Matrix4 => new Matrix4().compose(new Vector3(4, 0, -2), new Quaternion().setFromAxisAngle(new Vector3(0, 1, 0), 1.1), new Vector3(1, 1, 1));
...
describe('AnimatedInstances', () => {
  ...
  it('draws each part of a two-part rig at the character matrix × that part matrix', () => {
    ...
    expect(animation.parts.map((p) => [p.mesh.name, p.boneOffset])).toEqual([['body', 0], ['head', 2]]);
    ...
    animation.parts.forEach((part, k) => expectClose(partModel(instances, 2, k), new Matrix4().multiplyMatrices(character, part.matrix), part.mesh.name));
  });
  ...
  it('puts every vertex of both parts where three skins the same rig placed at the character matrix', () => {
    ...
    [original.body, original.head].forEach((mesh, k) => {
      ...
      for (let v = 0; v < position.count; v++) {
        ...
        const bound = new Vector4(position.getX(v), position.getY(v), position.getZ(v), 1).applyMatrix4(part.mesh.bindMatrix);
        ...
        const expected = mesh.applyBoneTransform(v, new Vector3(position.getX(v), position.getY(v), position.getZ(v))).applyMatrix4(mesh.matrixWorld);
        expect([drawn.x, drawn.y, drawn.z], `${mesh.name} vertex ${v}`).toEqual([expect.closeTo(expected.x, 4), expect.closeTo(expected.y, 4), expect.closeTo(expected.z, 4)]);
      }
    });
  });
