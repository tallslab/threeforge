...
export function liveRows(scenes: Partial<Record<SceneId, { naive?: BenchMetrics; optimized?: BenchMetrics }>>): string {
  return SCENE_IDS.map((id) => {
    ...
    return `<tr><td>${id}</td><td>${pair(n, o, (m) => String(m.sceneSubmissions))}</td><td>${pair(n, o, (m) => k(m.triangles))}</td><td>${pair(n, o, (m) => `${m.overdrawOpaque.toFixed(2)} / ${m.overdrawTransparent.toFixed(2)}`)}</td><td>${pair(n, o, (m) => k(m.skinnedVertices))}</td><td>${pair(n, o, mb)}</td><td>${pair(n, o, (m) => m.renderMs.toFixed(1))}</td><td>${pair(n, o, (m) => m.frameMs.toFixed(1))}</td></tr>`;
  }).join('');
}

/** `<tr>`s for the public table: device, backend, tier, fill rate, per scene submissions and frame ms, date. */
export function deviceRows(results: DeviceResult[]): string {
  return results
    .map((r) => {
      ...
      return `<tr><td>${esc(r.env.gpu)}<br><small>${esc(r.env.platform)}</small></td><td>${r.env.backend}</td><td>${r.env.tier}</td><td>${r.env.fillRateGPix === null ? '–' : r.env.fillRateGPix.toFixed(1)}</td>${cells}<td>${r.createdAt.slice(0, 10)}</td></tr>`;
    })
    .join('');
}
