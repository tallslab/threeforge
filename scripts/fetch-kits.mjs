...
const singles = [
  { name: 'ferrari', url: 'https://raw.githubusercontent.com/mrdoob/three.js/r186/examples/models/gltf/ferrari.glb', tags: ['car', 'many-materials', 'transparent'] },
  { name: 'waternormals', url: 'https://raw.githubusercontent.com/mrdoob/three.js/r186/examples/textures/waternormals.jpg', tags: ['texture'] },
];
...
for (const kit of kits) {
  try {
    ...
    const glbs = (await walk(dir)).filter((p) => p.toLowerCase().endsWith('.glb')).map((p) => relative(root, p));
    index.push({ name: kit.name, kind: 'kit', source: 'kenney.nl (CC0)', bytes, glbs, tags: ['kit', 'low-poly', 'shared-palette'] });
    console.log(`${kit.name.padEnd(30)} ${(bytes / 1e6).toFixed(1).padStart(6)} MB  ${glbs.length} glb`);
  } catch (e) {
